...
# AI Player
class Bot(Player):
    def __init__(self, id, limiter = 2,stepSize=0.1, exploreRate = 0.3):
                ...
                for i in range(-self.limiter, self.limiter+1):
                    for j in range(-self.limiter, self.limiter+1):
                        ...
                        if (ii,jj) not in state.data:
                            ...
                            if state.nextState(ii,jj).getHash() not in self.estimations:
                                if state.nextState(ii,jj).isEnd:
                                    if state.nextState(ii,jj).winner == id:
                                        self.estimations[state.nextState(ii,jj).getHash()] = 1
                                    else:
                                        self.estimations[state.nextState(ii,jj).getHash()] = 0
                                else : 
                                    self.estimations[state.nextState(ii,jj).getHash()] = 0.5
